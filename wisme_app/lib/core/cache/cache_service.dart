...
/// 
/// Advanced caching for audio files and other data with LRU eviction
library;
...
class CacheService {
  ...
  Future<File?> getCachedAudio(String topic, String coachVoice) async {
    try {
      ...
      AppLogger.error('Error checking cached audio: $e');
      return null;
    }
  }
  ...
  Future<void> _storeMetadata(String cacheKey, Map<String, dynamic> metadata) async {
    try {
      ...
      AppLogger.error('Error storing metadata: $e');
    }
  }
  ...
  Future<void> _updateLastAccessed(String cacheKey) async {
    try {
      ...
      AppLogger.error('Error updating last accessed: $e');
    }
  }
  ...
  Future<void> _cleanupOldestFiles(int bytesToFree) async {
    try {
      ...
      AppLogger.info('Cleaned up ${toRemove.length} cached files, freed ${freedBytes ~/ 1024}KB');
    } catch (e) {
